        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(navs,function(i,item){
                    ...
                    html += '<a href="javascript:;" lay-tips='+item.name+'  lay-direction="2" ><i class="layui-icon layui-icon-component"></i><cite>'+item.name+'</cite></a>';
                    //如果有第二级菜单
                    if(item.children !== undefined && item.children.length > 0){
                        html += '<dl class="layui-nav-child">';
                        $.each(item.children,function(j,item2){
                            html+='<dd>';
                            html += '<a href="javascript:;" >'+item2.name+'</a>';
                            //如果有三级菜单
                            if(item2.children !== undefined && item2.children.length > 0){
                                html += '<dl class="layui-nav-child">';
                                $.each(item2.children,function(k,item3){
                                    html += '<dd>'+
                                        '<a href="javascript:;" lay-href='+item3.url+'>'+item3.name+'</a>'+
                                        '</dd>';
                                });
                                html += '</dl>';
                            }
